...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const dedupedReactions = reactionsWithCounts.reduce(
      (acc, reaction) => {
        const existiongReaction = acc.find(
          (r) => r.value === reaction.value
        );

        if (existiongReaction) {
          existiongReaction.memberIds = Array.from(
            new Set([...existiongReaction.memberIds, reaction.memberId])
          )
        } else {
...
export const get = query({
  ...
  handler: async (ctx, args) => {
    ...
    const results = await ctx.db
      .query("messages")
      .withIndex(
        "by_channel_id_parent_message_id_conversation_id", (q) => q
